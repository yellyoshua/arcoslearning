export function mixColors(index) {
	let key = index;
	let key2 = index;
	const mixColors = [
		'rgba(233, 30, 99, 0.3)',
	...
	if (!key) {
		...
		key2 =
			Math.floor(Math.random() * (mixColors.length - 0)) + mixColors.length;
	}
	return {
		background: mixColors[key % mixColors.length],
		linear: `linear-gradient(90deg, ${mixColors[key % mixColors.length]} 40%, ${
			mixColors2[key2 % mixColors2.length]
		})`,
		mix: {
